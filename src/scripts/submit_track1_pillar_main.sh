...
TIME_LIMIT=${SBATCH_TIME:-96:00:00}
...
echo "============================================================"
echo "[Submit Track1 Pillar Main]"
...
JID_A=$(submit_variant "t1nq_pillar_a" "pillar_a" 128 96 2 rangeformer 192 3 1.5 5.0 0.25 2.0 1 coord_conditioned 192 0.24 0.24 64 128 128-128-192-256 2-2-2-3 128)
JID_B=$(submit_variant "t1nq_pillar_b" "pillar_b" 160 128 2 frnet 224 4 1.5 5.0 0.25 2.5 1 coord_conditioned 224 0.32 0.32 96 160 160-192-256-320 2-2-3-3 160)

MANIFEST="logs/${BASE_TAG}_track1_pillar_main_manifest.csv"
cat > "${MANIFEST}" <<CSV
submitted_at,job_id,label,variant,partition,time_limit,img_h,img_w,unprojection_mode,epochs_noquant,train_batch_size,latent_channels,base_channels,blocks_per_stage,fusion_variant,fusion_hidden,decoder_refine_blocks,recon_range_weight,recon_xyz_weight,recon_remission_weight,lambda_ray_consistency,pillar_size_x,pillar_size_y,pillar_pfn_hidden,pillar_pfn_out,pillar_bev_channels,pillar_bev_blocks,pillar_fpn_channels
$(date '+%Y-%m-%d %H:%M:%S %Z'),${JID_A},t1nq_pillar_a,pillar_a,${PARTITION},${TIME_LIMIT},${IMG_H},${IMG_W},${UNPROJECTION_MODE},${TRAIN_EPOCHS_NOQUANT},${TRAIN_BATCH_SIZE},128,96,2,rangeformer,192,3,1.5,5.0,0.25,2.0,0.24,0.24,64,128,"128,128,192,256","2,2,2,3",128
$(date '+%Y-%m-%d %H:%M:%S %Z'),${JID_B},t1nq_pillar_b,pillar_b,${PARTITION},${TIME_LIMIT},${IMG_H},${IMG_W},${UNPROJECTION_MODE},${TRAIN_EPOCHS_NOQUANT},${TRAIN_BATCH_SIZE},160,128,2,frnet,224,4,1.5,5.0,0.25,2.5,0.32,0.32,96,160,"160,192,256,320","2,2,3,3",160
CSV

echo "t1nq_pillar_a job_id=${JID_A}"
echo "t1nq_pillar_b job_id=${JID_B}"
