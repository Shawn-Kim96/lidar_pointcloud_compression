#!/bin/bash
# Parallel Stage1 sweep over ROI target downsampling mode.
# Usage:
#   sbatch src/scripts/run_stage1_roi_target_sweep.sh
#
# Array layout:
#   0 -> nearest
#   1 -> maxpool
...
#SBATCH --job-name=lidar_s1_rt_sweep
...
LOG_PREFIX="${DATE_TAG}_${BACKBONE}_solo_rt${ROI_TARGET_MODE}_j${JOB_TAG}_r${SLURM_ARRAY_TASK_ID}"
exec > >(tee -a "logs/${LOG_PREFIX}.out")
exec 2> >(tee -a "logs/${LOG_PREFIX}.err" >&2)

RUN_ID="j${JOB_TAG}_r${SLURM_ARRAY_TASK_ID}"
MODE_TAG="solo_rtsweep"
...
echo "Done Stage1 ROI target mode sweep task ${SLURM_ARRAY_TASK_ID} (${ROI_TARGET_MODE})"
